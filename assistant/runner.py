...
class AgentRunner:
    ...
    def run_cli(self, *, first_message=None):
        ...
        input_buffer = Buffer(completer=COMMANDS)
            ...
            def on_message(msg):
                if msg.metadata.get("chunk"):
                    if not prefix_printed[0]:
                        chat_fragments.append(('class:agent', f'{self.config.agent_name}: '))
        ...
        async def chat_loop():
            ...
            if first_message:
                chat_fragments.append(('class:user', f'{self.config.user_name}\uff1a'))
